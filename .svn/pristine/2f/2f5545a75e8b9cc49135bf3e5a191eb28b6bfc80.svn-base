...
//导入
function importIntoAdd(flag) {
	var switchhouseId = "";
	if(flag == 1){
		switchhouseId = $('#switchChangeId').val();
	}
	var url = Utils.getRootPath() + '/importExcel/switchChangeAdd?switchChangeId=' + switchhouseId;
	$('#myBatch').form('submit', {
		url : url,
		dataType : 'html',
		onSubmit : function() {
...
function exportExcelAdd() {
	...
	for (var i = 0; i < data.rows.length; i++) {
		$('#dg').datagrid('endEdit', i);
		$('#dg').datagrid('beginEdit', i);
		idStr += data.rows[i].SERIALNUMBER + ",";
		if (data.rows[i].CHANGENUMBER == null || data.rows[i].CHANGENUMBER == ''
				|| data.rows[i].CHANGENUMBER == 'undefiend') {
			num += "0.00,";
		} else {
			num += data.rows[i].CHANGENUMBER + ",";
		}
		goodsPrice += data.rows[i].GOODS_PRICE + ",";
		if (data.rows[i].REMARK == null || data.rows[i].REMARK == ''
			|| data.rows[i].REMARK == 'undefiend') {
			remarks += " ,";
		} else {
			remarks += data.rows[i].REMARK + ",";
		}
	}
	var url = Utils.getRootPath() + '/exportExcel/swichChangeExcelAdd?odd=' + odd
			+ '&idStr=' + idStr + '&num=' + num + '&goodsPrice=' + goodsPrice + '&remarks=' + remarks ;
