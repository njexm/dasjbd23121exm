	...
	$('#MyTree').tree(
			{
				...
				onClick : function(node) {
					if ($('#MyTree').tree('isLeaf', node.target)) {
						...
						//var childrenNodes=$('#MyTree').tree('getChildren',node.target);
						//var ids = node.id+",";
						//for (var int = 0; int < childrenNodes.length; int++) {
						//	ids += childrenNodes[int].id +",";
						//}
						var queryParams = {
							'goodsFile.id' : "",
							'goodsFile.goods_class.id' :node.id,
						};
						$('#dg').datagrid('options').queryParams = queryParams;
						$("#dg").datagrid('load');
						document.getElementById("treeValue").value = node.id;
