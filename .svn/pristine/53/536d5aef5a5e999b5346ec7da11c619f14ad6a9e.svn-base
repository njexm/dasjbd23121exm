...
function importIntoAdd() {
	...
	var url = Utils.getRootPath() + '/importExcel/importIntoReceive?ids=' + ids;
	$('#myBatch').form('submit', {
		url : url,
		dataType : 'html',
		onSubmit : function() {
...
function exportExcelAdd() {
	debugger;
	var odd = $('#purchaseReceiveOdd').val();
	var data = $('#dg').datagrid('getData');
	var idStr = "";
	var num = "";
	var goodsPrice = "";
	...
	var url = Utils.getRootPath() + '/exportExcel/exportExcelReceiveAdd?odd=' + odd
...
//导出
function exportExcel(purchaseReceiveOdd, id) {
	debugger;
	var data = $('#dg').datagrid('getData');
	var num = "";
	for (var i = 0; i < data.rows.length; i++) {
	...
	var url = Utils.getRootPath() + '/exportExcel/outPurchaseReceiveExcel?id='
			+ id + '&purchaseReceiveOdd=' + purchaseReceiveOdd
			+ '&num=' + num;
	...
	// 编辑导入
	function batchImport(purchaseReceiveId) {
		var ids = "";
		var data = $('#dg').datagrid('getData');
		...
		var url = Utils.getRootPath() + '/importExcel/savePRItemImport?purchaseReceiveId='
				+ purchaseReceiveId + '&ids=' + ids;
		$('#importBtn').text('导入中');
		$('#myBatch').form('submit', {
			url : url,
			dataType : 'html',
