...
function openAdd() {
	var url = Utils.getRootPath()
			+ '/supplierSettlement/supplierSettlementDo/gotoAdd';
	window.parent.initAdd(url, "新增供应商结算单", 1200, 560);
}
...
function destroyInfo() {
	...
	$.ajax({
		...
		url : Utils.getRootPath()
				+ '/supplierSettlement/supplierSettlementDo/deleteItem',
		data : {
			id : ids
		},
		async : false,
...
function save(formId) {
	...
	if (validateSubmit(formId)) {
		// 保存
		var id = $('#supplierSettlementId').val();
		...
		var url = Utils.getRootPath()
				+ '/supplierSettlement/supplierSettlementDo/save?id=' + id
...
function edit(title, x, y) {
	...
	var url = Utils.getRootPath()
			+ '/supplierSettlement/supplierSettlementDo/gotoEdit?id=' + row.ID;
	add(url, title, x, y);
}
...
function openCheck(title, x, y) {
	...
	var url = Utils.getRootPath()
			+ '/supplierSettlement/supplierSettlementDo/gotoCheck?id=' + row.ID;
	add(url, title, x, y);
}
...
function checkPass() {
	...
	var rows = JSON.stringify(data.rows);
	var url = Utils.getRootPath()
			+ '/supplierSettlement/supplierSettlementDo/checkPass?id=' + id
			+ '&rows=' + rows;
...
function CheckItemAjax(id, reason, type) {
	var url = Utils.getRootPath()
			+ '/supplierSettlement/supplierSettlementDo/check';
	$.ajax({
		type : "post",
		url : url,
		data : {
	...
	$.messager.confirm('确认 ', '确定提交审核？', function(r) {
		if (r) {
			$.ajax({
				type : "post",
				url : Utils.getRootPath()
						+ '/supplierSettlement/supplierSettlementDo/toCheck',
				data : {
					'id' : id
				},
				async : false,
	...
	$.messager.confirm('确认 ', '确定要删除 ？', function(r) {
		if (r) {
			$.ajax({
				type : "post",
				url : Utils.getRootPath()
						+ '/supplierSettlement/supplierSettlementDo/delete',
				data : {
					'id' : id
				},
				async : false,
...
// 新增时判断已有的供应商结算单状态
function provider() {
	var id = $('#provider').combobox('getValue');
	var url = Utils.getRootPath()
			+ '/supplierSettlement/supplierSettlementDo/provider?id=' + id;
	$.ajax({
		type : "post",
		url : url,
		async : false,
...
function searchData() {
	var queryParams = {
		'supplierSettlement.provider.id' : $('#provider').combobox('getValue')
	};
