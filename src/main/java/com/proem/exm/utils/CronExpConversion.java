...
public class CronExpConversion {
	   ...
	   /** 
		...
		public  static  String convertDateToCronExp(ZcOrder zcOrder,OrdersService orderservice)  {
	    	  String cronEx  =   "" ;
	    	  ...
	    	  List<TimeQuartz> timeQuartzList =orderservice.getListByObj(TimeQuartz.class, "");
	    	  if(timeQuartzList !=null && timeQuartzList.size()>0){
	    	  TimeQuartz timeQuartz =  timeQuartzList.get(0);
    		  timeQuartz.setPushType(pushType);
	    	  timeQuartz.setPushHz(pushHz);
	    	  timeQuartz.setPushDate(OrderDate);
	    	  timeQuartz.setPushStartDate(createTime);
	    	  timeQuartz.setPushEndDate( updateTime);
	    	  timeQuartz.setOrderCome(zcOrder.getOrderCome());
	    	  orderservice.updateObj(timeQuartz);
	    	  }else{
	    		  TimeQuartz timeQuartz=new TimeQuartz();
		    	  timeQuartz.setPushType(pushType);
		    	  timeQuartz.setPushHz(pushHz);
		    	  timeQuartz.setPushDate(OrderDate);
		    	  timeQuartz.setPushStartDate(createTime);
		    	  timeQuartz.setPushEndDate( updateTime);
		    	  timeQuartz.setOrderCome(zcOrder.getOrderCome());
		    	  orderservice.saveObj(timeQuartz);
	    	  }
